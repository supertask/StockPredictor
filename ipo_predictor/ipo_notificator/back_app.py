...
def display_stock(symbol, buy_signal_days, sell_signal_days, data, rsi_sell_threshold, rsi_buy_threshold):
    with st.container():
        st.markdown(f"<div style='border: 2px solid; padding: 10px; margin: 10px;'><h2>{symbol}</h2>", unsafe_allow_html=True)
...
# 列数を選択するセレクトボックスを追加
columns_option = st.selectbox('Select number of columns', [1, 2])
...
if st.button('Analyze'):
    ...
    if columns_option == 1:
        for symbol in symbols:
            ...
            #st.write('Buy Signals:', buy_signal_days[['Close', 'MACD', 'SignalLine', 'RSI']])
            #st.write('Sell Signals:', sell_signal_days[['Close', 'MACD', 'SignalLine', 'RSI']])
