...
class EdinetReportGetter:
    ...
    def __init__(self):
        ...
        self.searching_past_year = 0.5 #過去13年分を辿る
        self.is_debug = False
        ...
        self.DEBUG_DIR = os.path.join(self.TMP_DIR, 'debug_files')
    ...
    def download_and_extract_edinet_zip(self):
        ...
        #with open(self.edinet_csv_path, "r", encoding='shift_jis') as csvfile:
            ...
            for file_name in the_zip.namelist():
                ...
                if file_name.startswith('XBRL_TO_CSV/jpcrp') and file_name.endswith('.csv'):
                    with the_zip.open(file_name) as the_file:
                        ...
                        # デバッグモードでファイルを保存
                        if self.is_debug:
                            debug_file_path = os.path.join(self.DEBUG_DIR, file_name.replace('/', '_'))
                            with open(debug_file_path, 'wb') as debug_file:
                                debug_file.write(raw_data)
                            print(f"DEBUG: File saved as {debug_file_path}")
    ...
    def save_latest_document(self, group, doc_type_code, folder, doc_name, company_code4, company_name):
        ...
        # 有価証券届出の場合、事業内容などが省略されていることが多いのでエラーは出さない
        if self.is_debug or doc_type_code != '030':
            print(f"\033[91mError: No valid '事業の内容' found for {company_name} ({company_code4}) on doc_type_code = {doc_type_code}\033[0m")

    def download_and_save_document_metadata(self, doc_meta_path, tracking_days, edinet_to_company_dict):
        all_data = []
        today = datetime.today()
        end_datetime = today - timedelta(days=10 * 365)  # ここからデータが存在しない
